    ...
    <div className="w-full flex flex-wrap gap-4 mt-6 justify-start items-start">
      {campaigns.map((campaign) => (
        ...
        >
          ...
          <p className="text-sm flex w-full justify-between items-center pt-4">
            {" "}
            <span className="block">Status: {campaign.status}</span>{" "}
            <span className="block p-4 bg-background rounded-lg font-bold w-1/2">
              {" "}
              Jobs: {JSON.parse(campaign?.champain_json).length || 0}
            </span>{" "}
          </p>
        </div>
      ))}
