    ...
    {isRunnableFile(tab) && (
      <button
        className={`ml-auto px-2 py-1 text-sm ${
          isRunning || isAnyScriptRunning ? "bg-gray-500" : "bg-foreground"
...
const Page = () => {
    ...
    const checkScriptStatus = async () => {
      try {
        const response = await fetch(
          `http://localhost:3000/campaigns/${params.id}/script-status`
        );
        ...
        const scriptType =
          activeTab === "email_gen_script"
            ? "email_generation_script"
            : activeTab === "cleanup_script"
            ? "cleanup_script"
            : activeTab === "fetch_script"
            ? "fetch_script"
            : activeTab === "jobs_cleanup_script"
            ? "jobs_cleanup_script"
            : activeTab === "send_emails_script"
            ? "send_emails_script"
            : null;

        if (scriptType && status.isRunning) {
          const logsResponse = await fetch(
            `http://localhost:3000/campaigns/${params.id}/logs/${scriptType}`
          );
          if (logsResponse.ok) {
  ...
  const handleSave = async (value: string | undefined) => {
    ...
    try {
      ...
      const fieldName =
        activeTab === "raw_html"
          ? "champain_raw_html"
          : activeTab === "clean_json"
          ? "champain_json"
          : activeTab === "cleanup_script"
          ? "cleanup_script"
          : activeTab === "fetch_script"
          ? "fetch_script"
          : activeTab === "jobs_raw_html"
          ? "jobs_raw_html"
          : activeTab === "jobs_cleanup_script"
          ? "jobs_cleanup_script"
          : activeTab === "jobs_clean_json"
          ? "jobs_clean_json"
          : activeTab === "email_gen_script"
          ? "email_generation_script"
          : activeTab === "generated_emails_json"
          ? "generated_emails_json"
          : activeTab === "send_emails_script"
          ? "send_emails_script"
          : activeTab;
      ...
      const response = await fetch(
        `http://localhost:3000/campaigns/${params.id}`,
        {
          method: "PUT",
          headers: {
  ...
  const getContent = (tab: string) => {
    switch (tab) {
      ...
      case "cleanup_script":
        return (
          campaignData?.cleanup_script ||
          "// Cleanup script for campaign raw HTML"
        );
      case "clean_json":
      ...
      case "jobs_cleanup_script":
        return (
          campaignData?.jobs_cleanup_script ||
          "// Cleanup script for jobs raw HTML"
        );
      case "jobs_clean_json":
        return campaignData?.jobs_clean_json || "{}";
      case "email_gen_script":
        return (
          campaignData?.email_generation_script || "// Email generation script"
        );
      case "generated_emails_json":
  ...
  const handleTabSwitch = async (newTab: typeof activeTab) => {
    try {
      ...
      if (isRunnableFile(newTab)) {
        const scriptType =
          newTab === "email_gen_script"
            ? "email_generation_script"
            : newTab === "cleanup_script"
            ? "cleanup_script"
            : newTab === "fetch_script"
            ? "fetch_script"
            : newTab === "jobs_cleanup_script"
            ? "jobs_cleanup_script"
            : newTab === "send_emails_script"
            ? "send_emails_script"
            : null;
  ...
  const handleRunScript = async (scriptType: string) => {
    try {
      ...
      const apiScriptType =
        scriptType === "fetch_script"
          ? "fetch_script"
          : scriptType === "cleanup_script"
          ? "cleanup_script"
          : scriptType === "jobs_cleanup_script"
          ? "jobs_cleanup_script"
          : scriptType === "email_gen_script"
          ? "email_generation_script"
          : scriptType === "send_emails_script"
          ? "send_emails_script"
          : scriptType;
      ...
      const response = await fetch(
        `http://localhost:3000/campaigns/${params.id}/execute/${apiScriptType}`,
        {
          method: "POST",
        }
      );
  ...
  return (
    <div className="flex flex-col max-w-screen-2xl px-6 pt-6 m-auto min-h-screen font-[family-name:var(--font-geist-sans)]">
      ...
      <div className="flex gap-6 h-[calc(100vh-120px)]">
        ...
        <div className="w-1/3 overflow-scroll bg-white bg-opacity-10 border border-white rounded-lg p-6 backdrop-blur-sm">
            ...
            <div className="mb-4">
              ...
              <div className="space-y-2">
                ...
                <FileItem
                  ...
                  isRunning={
                    isScriptRunning && activeTab === "jobs_cleanup_script"
                  }
                  isAnyScriptRunning={isAnyScriptRunning}
                />
                <FileItem
            ...
            <div>
              ...
              <div className="space-y-2">
                <FileItem
                  ...
                  isRunning={
                    isScriptRunning && activeTab === "email_gen_script"
                  }
                  isAnyScriptRunning={isAnyScriptRunning}
                />
                <FileItem
                ...
                <FileItem
                  ...
                  onRun={() => handleRunScript("send_emails_script")}
                  isRunning={
                    isScriptRunning && activeTab === "send_emails_script"
                  }
                  isAnyScriptRunning={isAnyScriptRunning}
                />
              </div>
        ...
        <div className="w-2/3 flex flex-col gap-2">
          ...
          <div
            className={`bg-white bg-opacity-10 border border-white rounded-lg backdrop-blur-sm overflow-hidden flex flex-col ${
              showLogs && isRunnableFile(activeTab) ? "h-2/3" : "flex-1"
            }`}
          >
            {/* File Header */}
            ...
            >
              <div className="px-4 py-2 bg-foreground text-background font-mono text-sm flex justify-between items-center">
                <div className="flex items-center gap-2">
                  ...
                  {isScriptRunning && (
                    <span className="text-xs px-2 py-1 bg-yellow-500 rounded">
                      Running...
                    </span>
                  )}
                </div>
                ...
                <div className="flex-1 overflow-auto bg-background p-4 font-mono text-sm">
                  {scriptLogs.length === 0 ? (
                    ...
                    scriptLogs.map((log, index) => (
                      <div
                        key={index}
                        className={`whitespace-pre-wrap mb-1 border-b border-gray-800 pb-1 ${
                          log.level === "error"
                            ? "text-red-400"
                            : log.level === "debug"
                            ? "text-yellow-400"
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
          <div className="bg-background p-8 rounded-lg w-2/4">
            ...
            <div className="mb-4">
              ...
              <textarea
                ...
                onChange={(e) =>
                  setEditedData({
                    ...editedData,
                    champain_raw_html: e.target.value,
                  })
                }
                className="w-full p-2 border rounded-lg bg-background"
                rows={10}
            ...
            <div className="w-full border-t border-gray-300 pt-4">
              <h3 className="text-center text-lg font-bold mb-2 text-red-500">
                Danger Zone
              </h3>
              <button
